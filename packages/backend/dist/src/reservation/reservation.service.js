...
let ReservationService = class ReservationService {
    ...
    async create(createReservationData, userId) {
        const { venueId, dateStart, dateEnd, ...reservationData } = createReservationData;
        const availability = await this.checkAvailability(venueId, dateStart, dateEnd);
        if (!availability.available) {
            throw new common_1.ConflictException('Selected dates are already reserved.');
        }
        try {
    ...
    async getByVenue(venueId) {
        ...
        if (!reservations.length) {
            throw new common_1.NotFoundException(`No reservations found for venue with ID ${venueId}`);
        }
        return reservations;
    }
    async getByUser(userId) {
        ...
        if (!reservations.length) {
            throw new common_1.NotFoundException(`No reservations found for user with ID ${userId}`);
        }
        return reservations;
    }
