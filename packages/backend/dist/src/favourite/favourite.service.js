...
let FavouriteService = class FavouriteService {
    ...
    async getOne(venueFavouriteUserId) {
        ...
        if (!favourite) {
            throw new common_1.NotFoundException(`Favourite with ID ${venueFavouriteUserId} not found`);
        }
        return favourite;
    }
    async getByVenue(venueId) {
        ...
        if (!favourites.length) {
            throw new common_1.NotFoundException(`No favourites found for venue with ID ${venueId}`);
        }
        return favourites;
    }
    async getByUser(userId) {
        ...
        if (!favourites.length) {
            throw new common_1.NotFoundException(`No favourites found for user with ID ${userId}`);
        }
        return favourites;
    }
